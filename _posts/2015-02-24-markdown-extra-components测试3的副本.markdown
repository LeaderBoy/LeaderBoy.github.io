...
##测试
```swift
func resetSubscribeBadgeValue(key : String,by : Int,byInteractive : Bool) {
    let sectionData = AttentionSectionData()
    items.value = sectionData.data
    let badge = Int(BadgeValue.iOS)! + Int(BadgeValue.Android)! +  Int(BadgeValue.HTML)!
    //  self.tabBarItem.badgeValue = badge == 0 ? nil : String(badge)
    updateBadgeValue(key: key, value: by)
}
```
